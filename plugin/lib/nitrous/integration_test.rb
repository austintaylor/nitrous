  ...
  class IntegrationTest < RailsTest
    ...
    def click_link(url, method=:get)
      if method == :delete
        ...
        fail("No link found with url <#{url}> and method delete") if elements.empty? || !elements.any?{|element| element["onclick"] && element["onclick"].include?("m.setAttribute('name', '_method'); m.setAttribute('value', 'delete');")}
        delete url
        follow_redirect! if redirect?
        puts response.body if error?
        assert !error?
